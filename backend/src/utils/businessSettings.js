...
const withLegacyAliases = (settings, { activeBranchIds = [] } = {}) => {
  const branchIds = sanitizeBranchIds(
    settings?.authorizedBranches?.branchIds,
    activeBranchIds
  )

  const logo = {
    ...defaultBusinessSettings.logo,
    ...(isPlainObject(settings?.logo) ? settings.logo : {}),
  }
  ...
  const next = {
    ...
    business: {
      ...
      ...(isPlainObject(settings?.business) ? settings.business : {}),
    },
    general: {
      ...defaultBusinessSettings.general,
      ...(isPlainObject(settings?.general) ? settings.general : {}),
    },
    notifications: {
    ...
    appearance: {
      ...
      ...(isPlainObject(settings?.appearance) ? settings.appearance : {}),
    },
    order: {
      ...defaultBusinessSettings.order,
      ...(isPlainObject(settings?.order) ? settings.order : {}),
    },
    automation: {
      ...defaultBusinessSettings.automation,
      ...(isPlainObject(settings?.automation) ? settings.automation : {}),
    },
  }
...
export function mergeBusinessSettings(existingSettings = {}, options = {}) {
  ...
  const merged = {
    ...
    authorizedBranches: {
      branchIds: sanitizeBranchIds(
        authorizedBranches.branchIds,
        safeExisting.allowedBranchIds
      ),
    },
    appearance: {
      ...defaultBusinessSettings.appearance,
  ...
  return withLegacyAliases(merged, options)
}

export function buildIncomingBusinessSettings(payload = {}) {
  const safePayload = isPlainObject(payload) ? payload : {}
  const nested = isPlainObject(safePayload.settings) ? safePayload.settings : safePayload
  const incoming = isPlainObject(nested) ? { ...nested } : {}

  if (typeof safePayload.qrMenuEnabled === 'boolean') {
    incoming.qrMenu = {
      ...(isPlainObject(incoming.qrMenu) ? incoming.qrMenu : {}),
      enabled: safePayload.qrMenuEnabled,
    }
  }

  if (Array.isArray(safePayload.allowedBranchIds) && !isPlainObject(incoming.authorizedBranches)) {
    incoming.authorizedBranches = { branchIds: safePayload.allowedBranchIds }
  }
