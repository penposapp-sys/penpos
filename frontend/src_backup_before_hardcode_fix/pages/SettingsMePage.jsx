...
export default function SettingsMePage({ apiBase }) {
  ...
  const saveEmail = async (e) => {
    ...
    setSaving(true)
    try {
      const res = await api(`${apiBase}/me/email`, { method: 'PUT', data: { email, currentPassword: emailPw }, silent: true, skipBranchHeader: true })
      if (!res?.ok) {
        const code = res?.code
  ...
  const saveUsername = async (e) => {
    ...
    setSaving(true)
    try {
      const res = await api(`${apiBase}/me/username`, { method: 'PUT', data: { username: t, currentPassword: usernamePw }, silent: true, skipBranchHeader: true })
      if (!res?.ok) {
        const code = res?.code
  ...
  const savePassword = async (e) => {
    ...
    setSaving(true)
    try {
      const res = await api(`${apiBase}/me/password`, { method: 'PUT', data: { currentPassword: pwCurrent, newPassword: pwNext }, silent: true, skipBranchHeader: true })
      if (!res?.ok) {
        const code = res?.code
