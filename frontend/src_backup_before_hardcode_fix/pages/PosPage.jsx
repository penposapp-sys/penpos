...
export default function PosPage() {
  ...
  const submitItemNote = async (val) => {
    ...
    const res = await withLock(key, () => safeAction((signal) => api(`http://localhost:4000/api/pos/orders/${orderId}/items/${selectedItemForNote}/note`, { method: 'PUT', data: { note: val }, signal, silent: true })))
  ...
  const submitItemCancel = async (val) => {
    ...
    if (isDebounced(key, 250)) return
    const res = await withLock(key, () => safeAction((signal) => api(`http://localhost:4000/api/pos/orders/${orderId}/items/${selectedItemForCancel}/cancel`, { method: 'PUT', signal, silent: true })))
    const fresh = pickOrder(res)
    if (fresh) {
      setNote(fresh.note || '')
    }
      ...
      <div className="saleCartList" style={{ marginTop: 12 }}>
        <div style={{ display: 'grid', gap: 8 }}>
          ...
          const setItemQtyByRow = async (row, nextQty) => {
            ...
            const res = await withLock(lockKey, () => safeAction((signal) => api(`http://localhost:4000/api/pos/orders/${orderId}/items/${itemId}/quantity`, {
              method: 'PUT',
