...
export default function WalkInPosPage() {
  ...
  const setKitchenMode = async (next) => {
    ...
    const res = await withLock(key, () => safeAction(
      (signal) => api(`http://localhost:4000/api/pos/orders/${orderId}/kitchen-mode`, { method: 'PUT', data: { kitchenEnabled: Boolean(next) }, signal, silent: true }),
      { reload: false }
    ))
    if (!res) return
  ...
  const { busy: actionBusy, safeAction, reloadOrder } = useSafeOrderActions({
    ...
    setOrder,
    pickOrder,
    reloadUrlForOrderId: (id) => `http://localhost:4000/api/pos/orders/${id}`
  })
  ...
  const loadCategories = async () => {
    const res = await api('http://localhost:4000/api/tenant/categories?active=true')
  ...
  const loadItems = async (categoryId) => {
    const res = await api(`http://localhost:4000/api/tenant/menu-items?active=true${categoryId ? `&categoryId=${categoryId}` : ''}`)
    if (res?.success === false) {
      setItems([])
      return
    }
  ...
  useEffect(() => {
    ...
    const loadPaymentSettings = async () => {
      try {
        const res = await api('http://localhost:4000/api/tenant/payment-settings')
        if (res?.success === false) {
          setPayMethods([])
          return
        }
  ...
  const loadActiveOrders = async (opts = {}) => {
    ...
    try {
      ...
      params.set('limit', '50')
      const res = await api(`http://localhost:4000/api/pos/walkin/orders?${params.toString()}`, { silent: true, skipBranchHeader: true })
  ...
  useEffect(() => {
    ...
    const run = async () => {
      const res = await api(`http://localhost:4000/api/pos/orders/${selectedOrderId}`, { silent: true })
      if (!res?.ok) return
      const fresh = pickOrder(res)
      if (fresh) {
        setOrder(fresh)
  ...
  const startWalkInOrder = async () => {
    try {
      const res = await api('http://localhost:4000/api/pos/walkin/orders', {
        method: 'POST',
  ...
  const addItem = async (menuItemId) => {
    ...
    const result = await safeAction(
      (signal) => api(`http://localhost:4000/api/pos/orders/${orderId}/items`, { method: 'POST', data: { menuItemId }, signal, silent: true }),
      { reload: false }
    )
  ...
  const removeItem = async (menuItemId) => {
    ...
    const result = await safeAction(
      (signal) => api(`http://localhost:4000/api/pos/orders/${orderId}/items/${menuItemId}`, { method: 'DELETE', signal, silent: true }),
      { reload: false }
    )
    const fresh = pickOrder(result)
    if (fresh) {
  ...
  const submitItemNote = async (val) => {
    ...
    if (isDebounced(key, 250)) return
    const res = await withLock(key, () => safeAction(
      (signal) => api(`http://localhost:4000/api/pos/orders/${orderId}/items/${selectedItemForNote}/note`, { method: 'PUT', data: { note: val }, signal, silent: true }),
      { reload: false }
    ))
    const fresh = pickOrder(res)
    if (fresh) {
  ...
  const submitItemCancel = async (val) => {
    ...
    if (isDebounced(key, 250)) return
    const res = await withLock(key, () => safeAction(
      (signal) => api(`http://localhost:4000/api/pos/orders/${orderId}/items/${selectedItemForCancel}/cancel`, { method: 'PUT', signal, silent: true }),
      { reload: false }
    ))
    const fresh = pickOrder(res)
    if (fresh) {
  ...
  const saveNote = async () => {
    ...
    const res = await safeAction(
      (signal) => api(`http://localhost:4000/api/pos/orders/${orderId}/note`, { method: 'PUT', data: { note }, signal, silent: true }),
      { reload: false }
    )
    const fresh = pickOrder(res)
    if (fresh) {
  ...
  const cancelOrder = async () => {
    ...
    const res = await safeAction(
      (signal) => api(`http://localhost:4000/api/pos/orders/${orderId}/cancel`, { method: 'PUT', signal, silent: true }),
      { reload: false }
    )
  ...
  const closeSelectedOrder = async () => {
    ...
    const res = await safeAction(
      (signal) => api(`http://localhost:4000/api/pos/orders/${orderId}/close`, { method: 'PUT', signal, silent: true }),
      { reload: false }
    )
    if (!res) return
    const fresh = pickOrder(res)
  ...
  const sendKitchen = async () => {
    ...
    await safeAction(
      (signal) => api(`http://localhost:4000/api/pos/orders/${orderId}/send`, { method: 'PUT', data: payload, signal, silent: true }),
      { reload: false }
    )
    await reloadOrder().catch(() => null)
  }
  ...
  const saveCustomerName = async (rawName) => {
    ...
    const res = await safeAction(
      (signal) => api(`http://localhost:4000/api/pos/orders/${orderId}/customer`, { method: 'PUT', data: { customerName }, signal, silent: true }),
      { reload: false }
    )
    const fresh = pickOrder(res)
  ...
  const payOrder = async () => {
    ...
    const res = await safeAction(
      (signal) => api(`http://localhost:4000/api/pos/orders/${currentOrderId}/payments`, { method: 'POST', data: { method: paymentMethod, amount, note: paymentNote }, signal, silent: true }),
      { reload: false }
    )
    const fresh = pickOrder(res)
    if (fresh) {
  ...
  const applyDiscount = async () => {
    ...
    const res = await safeAction(
      (signal) => api(`http://localhost:4000/api/pos/orders/${currentOrderId}/discount`, { method: 'PUT', data: { discountPercent: Number(discountDraft) }, signal, silent: true }),
      { reload: false }
    )
    const fresh = pickOrder(res)
    if (fresh) {
  ...
  const deletePayment = async (paymentId) => {
    ...
    const res = await safeAction(
      (signal) => api(`http://localhost:4000/api/pos/orders/${currentOrderId}/payments/${paymentId}`, { method: 'DELETE', signal, silent: true }),
      { reload: false }
    )
    const fresh = pickOrder(res)
    if (fresh) {
  ...
  const searchAccounts = async (q) => {
    ...
    try {
      ...
      params.set('limit', '20')
      const res = await api(`http://localhost:4000/api/accounts?${params.toString()}`, { skipBranchHeader: true })
  ...
  const submitVeresiye = async () => {
    ...
    setVeresiyeBranchError('')
    try {
      const res = await api(`http://localhost:4000/api/pos/orders/${currentOrderId}/veresiye`, {
        method: 'POST',
        data: {
  ...
  const openPaymentModal = async () => {
    try {
      ...
      if (entityId && isValidObjectId(entityId)) {
        await api('http://localhost:4000/api/tenant/audit', {
          method: 'POST',
              ...
              <div style={{ display: 'grid', gap: 8 }}>
                {(() => {
                  ...
                  const setItemQtyByRow = async (row, nextQty) => {
                    ...
                    if (isDebounced(key, 250)) return
                    const res = await withLock(key, () => safeAction(
                      (signal) => api(`http://localhost:4000/api/pos/orders/${orderId}/items/${itemId}/quantity`, { method: 'PUT', data: { quantity: nextQty }, signal, silent: true }),
                      { reload: false }
                    ))
                    const fresh = pickOrder(res)
