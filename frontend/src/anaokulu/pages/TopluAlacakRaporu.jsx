...
export default function TopluAlacakRaporu({ selectedSchoolId = '', schoolOnly = false }) {
  ...
  const reportTenants = useMemo(() => schoolOnly
    ? (tenantCtx?.tenant ? [tenantCtx.tenant] : [])
    : (accessibleTenants || []), [schoolOnly, tenantCtx, accessibleTenants])
  ...
  const headers = schoolOnly ? ['Öğrenci', 'Sınıf', ...MONTHS, 'Yıllık Toplam'] : ['Okul', 'Öğrenci', 'Sınıf', ...MONTHS, 'Yıllık Toplam']

  const download = () => {
    const body = rows.map(row => schoolOnly
      ? [row.studentName, row.studentClass, ...report.months.map(month => row.monthly[month.period].amount), row.annualTotal]
      : [row.schoolName, row.studentName, row.studentClass, ...report.months.map(month => row.monthly[month.period].amount), row.annualTotal])
  ...
  return <div style={{ display: 'grid', gap: 14 }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
      <div><h3 style={{ margin: 0 }}>{schoolOnly ? 'Öğrenci Alacak Matrisi' : 'Toplu Öğrenci Alacak Matrisi'}</h3><div style={{ color: '#64748b', fontSize: 12, marginTop: 4 }}>Her satır öğrenci, her sütun gerçek vade ayıdır.</div></div>
    ...
    <div style={{ overflow: 'auto', border: '1px solid #e2e8f0', borderRadius: 10, background: '#fff' }}>
      {loading ? <div style={{ padding: 30 }}>Veriler yükleniyor...</div> : <table style={{ borderCollapse: 'collapse', minWidth: 1250, width: '100%' }}><thead><tr style={{ background: '#f8fafc' }}>{headers.map(header => <th key={header} style={{ ...cell, textAlign: 'right', fontWeight: 800 }}>{header}</th>)}</tr></thead><tbody>{rows.length === 0 ? <tr><td colSpan={headers.length} style={{ ...cell, textAlign: 'center' }}>Öğrenci bulunamadı.</td></tr> : rows.map(row => <tr key={`${row.schoolId}:${row.studentId}`}>{!schoolOnly && <td style={cell}>{row.schoolName}</td>}<td style={{ ...cell, fontWeight: 700 }}>{row.studentName}</td><td style={cell}>{row.studentClass}</td>{report.months.map(month => <td key={month.period} style={{ ...cell, textAlign: 'right', color: row.monthly[month.period].overdue ? '#b91c1c' : '#0f172a', background: row.monthly[month.period].overdue ? '#fff1f2' : '#fff' }}>{formatCell(row.monthly[month.period])}</td>)}<td style={{ ...cell, textAlign: 'right', fontWeight: 800 }}>{money(row.annualTotal)}</td></tr>)}</tbody></table>}
    </div>
  </div>
}
