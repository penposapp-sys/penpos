...
export default function AnaokuluUyelerPage() {
  ...
  const [editForm, setEditForm] = useState({
    name: '', email: '', username: '', role: 'staff', isActive: true
  })
  ...
  const load = async () => {
    ...
    setError('')
    try {
      const res = await api(`/api/tenant/staff${includeInactive ? '?includeInactive=true' : ''}`, {
  ...
  const openCreate = () => {
    setCreateForm({ name: '', email: '', username: '', password: '', role: 'staff' })
    setFormError('')
    setCreateOpen(true)
  }
  ...
  const onCreate = async (e) => {
    ...
    try {
      ...
      const res = await api('/api/tenant/staff', {
  ...
  const onEditSave = async (e) => {
    ...
    try {
      ...
      const res = await api(`/api/tenant/staff/${selected.id || selected._id}`, {
  ...
  const onPwdSave = async (e) => {
    ...
    try {
      ...
      const res = await api(`/api/tenant/staff/${selected.id || selected._id}/password`, {
  ...
  const onDeleteConfirm = async () => {
    ...
    setFormLoading(true)
    try {
      const res = await api(`/api/tenant/staff/${delOpen.id || delOpen._id}`, {
  ...
  const roleBadge = (role) => {
    ...
    if (role === 'tenant_admin') return { ...common, background: '#eef2ff', color: '#3730a3', text: 'Yönetici' }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ margin: 0, color: '#475569', fontSize: 14 }}>
            Yönetici ve personel hesaplarını buradan yönetin. Eklenen üyeler{' '}
            <code style={{ background: '#eef2ff', padding: '2px 6px', borderRadius: 6, color: '#3730a3', fontSize: 12 }}>/anaokulu/login</code>{' '}
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} style={{ ...td, textAlign: 'center', padding: '40px 12px', color: '#94a3b8' }}>
                  Henüz üye eklenmemiş. Yukarıdaki butondan ilk üyeyi ekleyin.
                </td></tr>
              ) : items.map(u => {
                ...
                return (
                  <tr key={String(u.id || u._id || u.email)}
                    ...
                    <td style={{ ...td, color: '#334155' }}>{u.username || '-'}</td>
      ...
      {createOpen && (
        <ModalWrap title="Yeni Üye Ekle" onClose={() => setCreateOpen(false)}>
          <form onSubmit={onCreate}>
            ...
            <div style={FieldCls}>
              ...
              <select style={InputCls} value={createForm.role}
                ...
                <option value="tenant_admin">👑 Yönetici (Müdür / Tam Yetki)</option>
              </select>
            </div>
            {formError && <div style={{
