...
export const deleteCustomerPayment = async (customerId, paymentId, reason) => {
  ...
  return api(`/api/magaza/customers/${encodeURIComponent(cid)}/payments/${encodeURIComponent(pid)}`, {
    method: 'DELETE',
...
export const getCustomerMovements = async (customerId) => {
  const cid = String(customerId || '').trim()
  return api(`/api/magaza/customers/${encodeURIComponent(cid)}/movements`, { silent: true })
}
...
export const getStockCounts = async (branchId, params = {}) => {
  ...
  const qs = q.toString()
  return api(`/api/magaza/stock-counts${qs ? `?${qs}` : ''}`, { silent: true, headers: { 'x-branch-id': bid } })
}

export const getStockCountDetail = async (branchId, id) => {
  const bid = String(branchId || '').trim()
  const sid = String(id || '').trim()
  return api(`/api/magaza/stock-counts/${encodeURIComponent(sid)}`, { silent: true, headers: { 'x-branch-id': bid } })
}
...
export const getSaleDetail = async (branchId, id) => {
  ...
  return api(`/api/magaza/sales/${encodeURIComponent(sid)}${qs}`, {
    silent: true,
