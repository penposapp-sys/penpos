...
export default function DeliveryOrderDetailPage() {
  ...
  useEffect(() => {
    ...
    const q = String(createForm.phone || createForm.customerName || '').trim()
    if (!q) {
      setCustomerSuggestions([])
      setCustomerLookupLoading(false)
      return
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await api(`/api/pos/delivery/customers/search?q=${encodeURIComponent(q)}&limit=8`, { silent: true, suppressBranchModal: true })
  ...
  const createOrder = async () => {
    setCreateOrderError('')
    const safeCustomerId = String(createForm.customerId || '').trim()
  ...
  const saveCustomerEdit = async () => {
    ...
    const res = await safeAction((signal) => api(`/api/pos/delivery/orders/${order.id}/customer`, {
      method: 'PUT',
      body: JSON.stringify({
        customerId: order?.deliveryCustomerId || '',
        customerName,
        phone,
        address,
...
{showList && (
      <aside className="card delivery-left-panel">
        ...
        <div className="delivery-orders-scroll scrollbar-hidden">
          {orders.map((o, idx) => (
            (() => {
            ...
            <div
              ...
              style={{
                ...
                borderRadius: 8,
                cursor: 'pointer',
                backgroundColor: isSelected ? 'var(--app-surface-soft, var(--panelElevated))' : 'var(--app-surface, var(--panel))',
                color: 'var(--text)'
              }}
            >
        ...
        {false && tab === 'delivered' && orders.length < totalCount && (
          <button className="btn" onClick={() => loadOrders('delivered', { page: page + 1, append: true })} disabled={loading}>
            Daha fazla yükle
          </button>
        )}
      ...
      {showDetail && (
      <div className="card delivery-detail-shell" style={{ display: 'grid', gridTemplateRows: 'auto 1fr', gap: 10, overflow: 'hidden' }}>
        {!order ? (
          ...
          <>
            ...
            <div className="saleStandard3Col delivery-detail-layout pos-grid" style={{ minHeight: 0, height: '100%' }}>
              ...
              <div className="card salePanel delivery-detail-products">
                <div className="delivery-detail-section-head" style={{ paddingBottom: 6, borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ display: 'grid', gap: 8 }}>
                  {(() => {
                    ...
                    const renderLine = (row, opts = {}) => {
                      ...
                      const itemId = row?.repr?.id || row?.repr?._id || row?.itemId || null
                      const isQtyLocked = qtyInflightRef.current.has(String(itemId || '')) || (qtyCooldownUntilRef.current.get(String(itemId || '')) || 0) > Date.now()
                      ...
                      return (
                        ...
                        >
                          ...
                          {isMobilePortrait && inlineCancelItemId === row.itemId && (
                            <div style={{ marginTop: 10, display: 'grid', gap: 8 }}>
                              <textarea className="input" rows={3} placeholder="İptal sebebi" value={inlineCancelReason} onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()} onChange={(e) => setInlineCancelReason(e.target.value)} />
                              <div className="stackRow">
                        ...
                        {payments.map((p) => (
                          <div key={p._id || p.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'flex-start' }}>
                            <div style={{ display: 'grid' }}>
                              ...
                              <div style={{ fontWeight: 600 }}>{paymentMethodLabel(p)}</div>
                            </div>
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          ...
          {!!(customerLookupLoading || customerSuggestions.length > 0) && (
            <div className="card" style={{ borderColor: 'var(--border)', display: 'grid', gap: 8, padding: 10 }}>
              <div style={{ fontSize: 12, color: 'var(--muted)' }}>{customerLookupLoading ? 'Kayıtlı müşteriler aranıyor...' : 'Kayıtlı müşteriler'}</div>
              {(customerSuggestions || []).map((customer) => (
                <button key={customer.id} type="button" className="btn" onClick={() => applySuggestedCustomer(customer)} style={{ justifyContent: 'flex-start', textAlign: 'left' }}>
      ...
      <Modal open={customerEditOpen} onClose={() => setCustomerEditOpen(false)} title="Müşteri Düzenle">
        <div style={{ display: 'grid', gap: 10 }}>
          ...
          <div className="card" style={{ borderColor: 'var(--border)', display: 'grid', gap: 10, padding: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button type="button" className="btn btn--toggle" aria-pressed={customerEditForm.deliveryPaymentStatus === 'already_paid'} onClick={() => setCustomerEditForm({ ...customerEditForm, deliveryPaymentStatus: 'already_paid', deliveryPaymentMethod: '' })} disabled={busy}>Ödemesi alındı</button>
              <button type="button" className="btn btn--toggle" aria-pressed={customerEditForm.deliveryPaymentStatus === 'pay_on_delivery'} onClick={() => setCustomerEditForm({ ...customerEditForm, deliveryPaymentStatus: 'pay_on_delivery', deliveryPaymentMethod: customerEditForm.deliveryPaymentMethod || pickInitialPaymentMethod(payMethods, '') })} disabled={busy}>Kapıda ödeme</button>
            </div>
            {customerEditForm.deliveryPaymentStatus === 'pay_on_delivery' && (
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {(payMethods || []).map((method) => {
                  const methodKey = String(method?.key || method?.id || '')
                  const active = String(customerEditForm.deliveryPaymentMethod || '') === methodKey
                  return (
                    <button key={methodKey} type="button" className="btn btn--toggle" aria-pressed={active} onClick={() => setCustomerEditForm({ ...customerEditForm, deliveryPaymentMethod: methodKey })} disabled={busy}>
                      {method?.name || methodKey}
                    </button>
                  )
                })}
      ...
      <PaymentCollectionModal
        ...
        showVeresiye={!creditAccountsDisabled && canCreateVeresiye}
        onOpenVeresiye={openVeresiye}
      />
      <Modal open={false && payOpen} onClose={() => setPayOpen(false)} title="Ödeme Al">
          ...
          <div className="card" style={{ borderColor: 'var(--border)' }}>
            ...
            {selectedPaymentIsCash && (
              ...
              <div className="payment-actions">
                ...
                <button className="btn" onClick={openVeresiye} disabled={!canCreateVeresiye || creditAccountsDisabled || busy || balanceDue <= 0.01}>
                  Veresiye Yap
                </button>
            ...
            {(accountResults || []).map(a => (
              <div
                ...
                style={{
                  ...
                  borderRadius: 8,
                  cursor: 'pointer',
                  background: selectedAccount?.id === a.id ? 'color-mix(in srgb, var(--theme-accent, #2563eb) 18%, transparent)' : 'transparent'
                }}
              >
